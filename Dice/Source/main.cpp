...
void My_Init()
{
	...
	// Load shader file
	char** vertexShaderSource = loadShaderSource("dice.vertex");
	char** fragmentShaderSource = loadShaderSource("dice.fragment");
...
void My_Display()
{
	...
	glUniform1i(glGetUniformLocation(program, "plane"), 0);
	dg.setView(view);
	dg.draw(program);
	glUniformMatrix4fv(um4mv, 1, GL_FALSE, value_ptr(view * mat4()));
	glUniform1i(glGetUniformLocation(program, "plane"), 1);
	plane.draw();
...
void My_SpecialKeys(int key, int x, int y)
{
	...
	case GLUT_KEY_PAGE_UP:
		break;
	case GLUT_KEY_LEFT:
		dg.setPos(dg.getPos() - vec3(0.0, 0.0, 0.5));
		dg.setDir(0);
		break;
	case GLUT_KEY_RIGHT:
		dg.setPos(dg.getPos() + vec3(0.0, 0.0, 0.5));
		...
		break;
	case GLUT_KEY_UP:
		dg.setPos(dg.getPos() + vec3(0.5, 0.0, 0.0));
		dg.setDir(1);
		break;
	case GLUT_KEY_DOWN:
		dg.setPos(dg.getPos() - vec3(0.5, 0.0, 0.0));
